...
int WiFiClient::ns_connect(const char *host, uint16_t port)
{
  if (_sock != NO_SOCKET_AVAIL)
  {
    stop();
  ...
  if (_sock != NO_SOCKET_AVAIL)
  {
    ServerDrv::startClient(host, strlen(host), uint32_t(0), port, _sock, NS_MODE);
...
int WiFiClient::ns_connectSSL(const char *host, uint16_t port, bool verify)
{
 
  if (_sock != NO_SOCKET_AVAIL)
  {
    if(verify)
      ServerDrv::startClient(host, strlen(host), uint32_t(0), port, _sock, NS_TLS_VERIFY_MODE);
    else
      ServerDrv::startClient(host, strlen(host), uint32_t(0), port, _sock, NS_TLS_MODE);
